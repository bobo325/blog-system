...
def package_import(package, excludes=None):
    ...
    try:
        ...
        files = [x for x in get_dir_files_with_py(package_path) if x not in excludes]
        packages = []
        for file in files:
...
def get_abs_path(*args):
    ...
    根据args从根目录获取文件的绝对路径
    :param args: 例如
    get_abs_path("1","2","3.xml") -> ${root_dir}/1/2/3.xml
    get_abs_path() -> ${root_dir}
    :return: 文件的绝对路径
    """
    previous_path = "/../../"
